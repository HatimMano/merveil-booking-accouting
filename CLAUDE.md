...
## Known issues / notes
- ⚠️ **Fichier redéposé (même nom, contenu différent) = le journal d'idempotence ne protège PAS** : la clé porte le hash md5 du contenu → nouveau hash = tous les batches re-postés, y compris ceux déjà en compta (aucun DELETE possible par API). Cas réel 02/09 : `260814` redéposé avec 25 payouts de plus — avant tout run, **parser les deux versions et comparer les payout_ids au `posting_journal`** (zéro recouvrement ce jour-là, run OK). Le rejeu « fichier corrigé » suppose que les anciennes écritures ont été supprimées côté Pennylane d'abord (cas Iavotsoa).
- ⭐ **Lire le dossier Drive hors pipeline** (debug) : impersonation de la SA — `gcloud auth print-access-token --impersonate-service-account=booking-pipeline-sa@… --scopes=https://www.googleapis.com/auth/drive` puis API Drive REST (binding `serviceAccountTokenCreator` accordé à hatim@ le 02/09). L'archive d'un fichier va dans `Archive {date du payout le plus récent}` (get_or_create → un complément de semaine rejoint le dossier existant).
...
- ⚠️⚠️ **Ces référentiels ne portent PAS le code analytique.** Dans l'API v2 la ventilation analytique est accrochée à la **FACTURE** (`/supplier_invoices/{id}/categories`), **jamais** à la ligne d'écriture — `ledger_entry_lines.categories` est vide sur 100 % des 12 000 lignes scannées (juillet→septembre 2026). Le plan est bon : 145 catégories, 5 axes (Nature · **Adresse = par appartement** · Source Revenus = par canal · Projets transverses · Compta Intern) et **la saisie est excellente : 98-100 % des factures fournisseurs COMPTABILISÉES sont catégorisées, tous mois de 2025-03 à 2026-08, en nombre comme en montant** (mesure corrigée 21/08 après-midi, cf. ADR — le « 26-38 % » du matin était un artefact de sonde : 429 comptés « sans catégorie » + tête de `sort=-date` polluée par les documents **sans date** `archived`, jamais comptabilisés). **Lot 2 débloqué** (≈1 j : 1 appel `/categories` par facture, ~19 k de rattrapage, pattern `references.py`) — en attente du GO / des routines cibles de Mickael.
- ⚠️ Finding annexe (21/08) : les factures **sans date ne sont jamais ingérées** dans `raw_supplier_invoices` — la logique d'overlap d'`invoices_full.py` saute `date` NULL. Correct au fond (ce sont des documents non comptabilisés, boîte de réception Pennylane), mais à savoir avant de comparer des comptages avec l'UI Pennylane.
- ⚠️ **Piège de sonde API à retenir** : `sort=-date` sert les **NULL en premier** → la « tête » de la liste n'est pas « les plus récentes » mais les non-datées ; et dans toute mesure de taux, une erreur HTTP (429…) doit être **exclue du dénominateur**, pas comptée comme un négatif.
...
- Script CLI : `pennylane/invoice_categories.py` — ids sondés **depuis BQ** (`raw_{supplier,customer}_invoices`), `--days N` (défaut 90, aligné overlap Lot B) ou `--all` (backfill). 4 workers, retry 429 via `PennylaneGLClient._get`.
- Cloud Run Job `invoice-categories-pull` (task-timeout **1800**, retries 2) + scheduler `invoice-categories-pull-daily` **6h45 Paris**.
- Table `pennylane.raw_invoice_categories` : 1 row par (invoice_kind, invoice_id, category_id), `category_label`/`category_group_id` **dénormalisés** (résistent à une catégorie supprimée du plan). **Remplacement transactionnel par facture sondée** (DELETE + INSERT) : une catégorie retirée côté Pennylane disparaît au run suivant.
- ⚠️ **Une sonde en échec après retries sort du lot** — ni écrite ni purgée (sinon un incident réseau effacerait des assignations existantes). Les 404 = factures supprimées côté Pennylane mais conservées en BQ (le MERGE du Lot B ne supprime jamais) : normales, écartées.
- ⚠️ Le rythme soutenable de l'API est ~5 sondes/s (429 au-delà) → backfill complet ~65 min (fait en local le 21/08, ~19 k sondes). Le daily 90j ≈ 1 500 sondes tient largement dans le timeout.
- Aval dbt : `stg_pennylane__categories` (plan + axe) + `stg_pennylane__invoice_categories` (assignation + axe + `code_etablissement_analytique` dérivé sur l'axe Adresse — codes **établissement**, même clé que `dash_finance_loyer_charges_taxes`, PAS des apartment_codes). Les marts de contrôle de gestion attendent les routines de Mickael.
